...
func Map(c context.Context, m message.Message[string, string]) ([]Entity, error) {
	return []Entity{
		{
			Id:           m.Key,
			KeyContent:   m.Key,
...
func Runtime() runtime.Runtime {
	materialiseConfiguration := flows.MaterialisePostgresqlFunctionConfiguration[Entity]{
		...
		MaterialiseMapFunction: materialise.ConvertOneToOne(Map, format.String(), format.String()),
	}
